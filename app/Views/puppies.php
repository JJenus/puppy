<?= $this->extend($config->viewLayout) ?>
  ...
  <?= $this->section('pageScripts') ?>
    <script src="<?= base_url() ?>/assets/js/vue.min.js"></script>
		...
		<script src="<?= base_url() ?>/assets/js/app/puppies.vue.js"></script>
  <?= $this->endSection() ?> 
  ...
  <!-- Breadcrumb -->
      ...
      <section class="container mt-4 mb-lg-5 pt-lg-2 pb-5">
        ...
        <div class="d-flex align-items-center justify-content-between mb-4 pb-1 pb-md-3">
          <h1 class="mb-0">Welcome to puppy.com</h1>
          <div class="d-flex d-none align-items-center ms-sm-4 ms-3">
          ...
          <div class="col-xl-9 col-lg-8">
            <div class="pe-xl-5">
              <div v-if="puppies" class="masuonry-grid row row-cols-sm-2 row-cols-1 g-4">
                <puppy-details v-for="puppy in puppies" :puppy="puppy" :key="puppy.id"></puppy-details>
              </div>

              <!-- Load more button -->
              <button id="show-more-btn" v-if="search.result.length === search.limit" @click="loadPuppies(search.limit, puppies.length)" class="btn btn-lg btn-outline-primary w-100 mt-4">
